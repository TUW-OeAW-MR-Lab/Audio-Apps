/* define_loudspeakerscart.c

written by Ville Pulkki 1999 Helsinki University of Technology and
Unversity of California at Berkeley
See copyright in file with name COPYRIGHT

Modified by Michael Mihocic and Piotr Majdak
...
typedef struct { /* distance value is 1.0 == unit vectors */
  float x;       /* cartesian coordinates (normalised) */
  float y;
  float z;
  float azi; /* azimuth derived from cartesian (used internally for 2-D) */
  float ele; /* elevation derived from cartesian (used internally) */
...
int any_ls_inside_triplet(int a, int b, int c, t_ls lss[MAX_LS_AMOUNT],
                          int ls_amount);
...
void define_loudspeakerscart_setup(void) {
  ...
  class_addmethod(def_ls_class, (t_method)def_ls_read_directions,
...
{ /* calculate and print out chosen loudspeaker sets and corresponding  matrices
  ...
  if (x->x_ls_read == 1) {
    if (x->x_ls_amount < x->x_dimension) {
      ...
      post("dimension x1 y1 [z1] x2 y2 [z2]...");
      post("dimension == 2 for horizontal ls arrays (x y per speaker)");
      post("dimension == 3 for 3-D ls arrays (x y z per speaker)", 0);
    }
  } else {
    post("define-loudspeakers: Error in loudspeaker direction data", 0);
    post("dimension x1 y1 [z1] x2 y2 [z2]...", 0);
    post("dimension == 2 for horizontal ls arrays (x y per speaker)", 0);
    post("dimension == 3 for 3-D ls arrays (x y z per speaker)", 0);
  }
}
...
{
  long l1 = 0, l2 = 0, l3 = 0, i;
  t_ls_set *trip_ptr, *tmp_ptr, *prev;
  if (x->x_ls_read == 0) {
    post("define_loudspeakerscart: Define loudspeaker directions first!", 0);
    return;
  }

  if (x->x_dimension == 2) {
    post("define_loudspeakerscart: Can't specify loudspeaker triplets in 2-D "
         "setup!",
         0);
    return;
  }
...
void def_ls_read_directions(t_def_ls *x, t_symbol *s, int ac, t_atom *av)
/* when loudspeaker cartesian directions come in a message
   Format 3-D: ls-directions 3 x1 y1 z1 x2 y2 z2 ...
   Format 2-D: ls-directions 2 x1 y1 x2 y2 ...
   Example: ls-directions 3 1 1 1 2 -3 0.5
*/
{
  ...
  pointer = 1;
  /* each loudspeaker needs x->x_dimension coordinate values */
  x->x_ls_amount = (ac - 1) / x->x_dimension;

  for (i = 0; i < x->x_ls_amount; i++) {
    /* read X coordinate */
    if (av[pointer].a_type == A_FLOAT)
      x->x_ls[i].x = av[pointer].a_w.w_float;
    else {
      post("define-loudspeakers: Error in loudspeaker cartesian data (X)!", 0);
      newlyread = 0;
      return;
    }
    pointer++;
    ...
    /* read Y coordinate */
    if (av[pointer].a_type == A_FLOAT)
      x->x_ls[i].y = av[pointer].a_w.w_float;
    else {
      post("define-loudspeakers: Error in loudspeaker cartesian data (Y)!", 0);
      newlyread = 0;
      return;
    }
    pointer++;
    ...
    if (x->x_dimension == 3) {
      /* read Z coordinate */
      if (av[pointer].a_type == A_FLOAT)
        x->x_ls[i].z = av[pointer].a_w.w_float;
      else {
        post("define-loudspeakers: Error in loudspeaker cartesian data (Z)!",
             0);
        newlyread = 0;
        return;
      }
      ...
      x->x_ls[i].z = 0.0; /* 2-D case: Z is zero */
    }
  }

  if (newlyread == 1) {
    x->x_ls_read = 1; /* preprocess data: normalise cartesian vectors */
    for (i = 0; i < x->x_ls_amount; i++)
      ls_cart_normalize(&x->x_ls[i]);
    trip_ptr = x->x_ls_set;
    prev = NULL;
...
/* normalise cartesian loudspeaker vector to unit sphere */
{
  ...
  {
    float pi = 3.1415927f;
    float atorad = (2.0f * pi / 360.0f);
    float dist, atan_y_per_x, atan_x_pl_y_per_z;
    if (ls->x == 0.0f)
      atan_y_per_x = pi / 2.0f;
    else
      atan_y_per_x = atanf(ls->y / ls->x);
    ls->azi = atan_y_per_x / atorad;
    if (ls->x < 0.0f)
      ls->azi += 180.0f;
    dist = sqrtf(ls->x * ls->x + ls->y * ls->y);
    if (ls->z == 0.0f)
      atan_x_pl_y_per_z = 0.0f;
    else
      atan_x_pl_y_per_z = atanf(ls->z / (dist > 0.0001f ? dist : 0.0001f));
    ls->ele = atan_x_pl_y_per_z / atorad;
  }
}

static void *def_ls_new(t_symbol *s, int ac,
                        t_atom *av) /* create new instance of object */
{
  long i, pointer;
  t_def_ls *x;
  ...
  if (x->x_dimension < 2 || x->x_dimension > 3) {
    /* no valid dimension given as creation argument - that's OK,
       directions will be set via ls-directions message */
    x->x_dimension = 3; /* default */
  ...
  /* read loudspeaker cartesian coordinates */
  for (i = 0; i < x->x_ls_amount; i++) {
    if (av[pointer].a_type == A_FLOAT)
      x->x_ls[i].x = av[pointer].a_w.w_float;
    else {
  ...
  x->x_triplets_specified = 0;
  x->x_outlet0 =
      outlet_new(&x->x_ob, gensym("list")); /* create a (list) outlet */
  x->x_ls_set = NULL;
...
Auditory Displays -98.*/
{
  ...
  while (trip_ptr != NULL) {
    ...
    j = trip_ptr->ls_nos[1];
    k = trip_ptr->ls_nos[2];
    if (connections[i][j] == 0 || connections[i][k] == 0 ||
        connections[j][k] == 0 ||
...
float vol_p_side_lgth(int i, int j, int k, t_ls lss[MAX_LS_AMOUNT]) {
  ...
  lgth = (fabsf(vec_angle(lss[i], lss[j])) + fabsf(vec_angle(lss[i], lss[k])) +
...
/* Calculates the inverse matrices for 3D */
{
  ...
  list_length = triplet_amount * 21 +
                2; /* was: + 3, pure data doesn't like errors on list_length */
  ...
  while (tr_ptr != NULL) {
    ...
    SETFLOAT(&at[pointer], lp1->x);
    pointer++;
    SETFLOAT(&at[pointer], lp2->x);
    pointer++;
    SETFLOAT(&at[pointer], lp3->x);
    pointer++;
    SETFLOAT(&at[pointer], lp1->y);
    pointer++;
    SETFLOAT(&at[pointer], lp2->y);
    pointer++;
    SETFLOAT(&at[pointer], lp3->y);
    pointer++;
    SETFLOAT(&at[pointer], lp1->z);
    pointer++;
    SETFLOAT(&at[pointer], lp2->z);
    pointer++;
    SETFLOAT(&at[pointer], lp3->z);
    pointer++;
   ...
   matrices and stores the data to a global array*/
{
  ...
  int amount = 0;
  float inv_mat[MAX_LS_AMOUNT]
               [4]; /* In 2-D ls amount == max amount of LS pairs */
  float *ptr;
  float *ls_table;
  t_ls *lss = x->x_ls;
  ...
  for (i = 0; i < (ls_amount - 1); i++) {
    if ((lss[sorted_lss[i + 1]].azi - lss[sorted_lss[i]].azi) <= (180 - 10)) {
      if (calc_2D_inv_tmatrix(lss[sorted_lss[i]].azi,
                              lss[sorted_lss[i + 1]].azi, inv_mat[i]) != 0) {
        exist[i] = 1;
        amount++;
      }
    }
  ...
  if (((6.283 - lss[sorted_lss[ls_amount - 1]].azi) + lss[sorted_lss[0]].azi) <=
      (180 - 10)) {
...
void sort_2D_lss(t_ls lss[MAX_LS_AMOUNT], int sorted_lss[MAX_LS_AMOUNT],
                 int ls_amount)
/* sort loudspeakers according to azimuth angle (derived from cartesian) */
{
  int i, j, index = 0;
  float tmp, tmp_azi;
  float rad2ang = 360.0 / (2 * 3.141592);

  /* ls_cart_normalize was already called; just recompute azi from cart x,y */
  for (i = 0; i < ls_amount; i++) {
