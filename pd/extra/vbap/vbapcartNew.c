...
#define MAX_LS_SETS 385				/* maximum number of loudspeaker sets (triplets or pairs) allowed */
#define MAX_LS_AMOUNT 200			/* maximum amount of loudspeakers, can be increased */

typedef struct vbapcart				/* This defines the object as an entity made up of other things */
{
	t_object x_ob;				
...
void new_spread_dir(t_vbapcart *x, float spreaddir[3], float vscartdir[3], float spread_base[3]);
...
static void *vbapcart_new(t_symbol *s, int ac, t_atom *av); /* using A_GIMME - typed message list */
...
{
	vbapcart_class = class_new(gensym("vbapcart"), (t_newmethod)vbapcart_new, 0, (short)sizeof(t_vbapcart), 0, A_GIMME, 0); 
	/* vbapcart_new = creation function, A_DEFLONG = its (optional) arguement is a long (32-bit) int */

/* max methods ... */

/*	addbang((method)vbapcart_bang);			the procedure it uses when it gets a bang in the left inlet */
/*	addint((method)vbapcart_int);			the rocedure for an int in the left inlet (inlet 0) */
/*	addinx((method)vbapcart_in1, 1);		the rocedure for an int in the right inlet (inlet 1) */
/*	addinx((method)vbapcart_in2, 2);		the rocedure for an int in the right inlet (inlet 2) */
/*	addinx((method)vbapcart_in3, 3); */
/*	addmess((method)vbapcart_matrix, "loudspeaker-matrices", A_GIMME, 0); */

/* pure data: */

	class_addbang(vbapcart_class, vbapcart_bang);
	class_addfloat(vbapcart_class, vbapcart_int);
	class_addmethod(vbapcart_class, (t_method)vbapcart_matrix, gensym("loudspeaker-matrices"), A_GIMME, 0);
}
...
void vbapcart_int(t_vbapcart *x, t_float n) /* x = the instance of the object, n = the int received in the right inlet */
{
...
void vbapcart_in1(t_vbapcart *x, long n)				/* x = the instance of the object, n = the int received in the right inlet */
...
void vbapcart_in2(t_vbapcart *x, long n)				/* x = the instance of the object, n = the int received in the right inlet */
...
void vbapcart_in3(t_vbapcart *x, long n)				/* x = the instance of the object, n = the int received in the right inlet */
/* spread amount */
{
	if (n<0) n = 0;
	if (n>100) n = 100;
