...
/*			if(av[0].a_type == A_LONG) 
	...
	post("define-loudspeakers: Directions of %d loutspeakers saved",x->x_ls_amount);
}
     ...
     Auditory Displays -98.*/
{
  ...
  /* Warning this function is buggy!!!! For larger arrays, define triplets with ls-triplets*/
...
{  
  ...
  //post("define-loudspeakers: send bang to vbap",0);
